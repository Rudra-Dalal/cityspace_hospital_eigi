...
async def start_booking_flow(
    ...
    """Initiate booking flow step 1: Hospital branch selection."""
    if not patient or not session.patient_id:
        await adapter.send_message(
            chat_id=chat_id,
            text=(
                "🔒 *Account Verification Required*\n\n"
                "To book an appointment, please link your existing CityCare account or register as a new patient first\\."
            ),
...
async def handle_booking_text_message(
    ...
    """Handle text input when patient is typing reason or symptoms."""
    if session.current_flow != TelegramFlowType.BOOKING.value:
        return False
    if session.flow_step != "enter_reason":
        return False

    reason_clean = text.strip()
    if not reason_clean:
